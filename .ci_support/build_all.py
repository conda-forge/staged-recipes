...
def build_all(recipes_dir, arch):
    folders = list(
        filter(
            lambda d: os.path.isdir(os.path.join(recipes_dir, d)),
            os.listdir(recipes_dir),
        )
    )
    if not folders:
    ...
    if has_meta_yaml:
        print(
            "Building {} with {} using conda-build".format(
...
def check_recipes_in_correct_dir(root_dir, correct_dir):
    ...
    for path in chain(
        Path(root_dir).rglob("meta.yaml"), Path(root_dir).rglob("recipe.yaml")
    ):
