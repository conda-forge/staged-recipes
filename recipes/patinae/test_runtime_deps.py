...
def parse_macho_deps(target: Path) -> list[str]:
    ...
    # `otool -L` prints a dylib's own LC_ID_DYLIB install name as the
    # first entry. It is the library's identity, not a runtime dependency.
    if target.suffix == ".dylib" and deps:
        deps = deps[1:]
