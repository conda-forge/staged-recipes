...
def main [--debug (-d)] {
    ...
    for path in $possible_paths {
        if ($path | path exists) {
            ...
            # Look for asciidoctor-revealjs related directories
            try {
                let contents = (ls $"($path)" --full-paths 2>/dev/null | default [])
                let gem_dirs = ($contents | where type == dir | get name | default [])
                let revealjs_patterns = ["asciidoctor-revealjs", "gems", "specifications"]

                for pattern in $revealjs_patterns {
        ...
        for spec_dir in $spec_dirs {
            if ($spec_dir | path exists) {
                try {
                    ...
                    let revealjs_specs = ($spec_files | where name =~ "asciidoctor-revealjs")

                    if ($revealjs_specs | length) > 0 {
                        print $"[OK] Found gem specification files: ($revealjs_specs | get name | str join ', ')"
                        $found_spec = true
                    } else {
                        print $"[INFO] No asciidoctor-revealjs specification files in ($spec_dir)"
                    }
                } catch {
        ...
        for lib_path in $lib_paths {
            if ($lib_path | path exists) {
                try {
                    ...
                    let revealjs_dirs = ($gem_dirs | where name =~ "asciidoctor.*reveal")

                    if ($revealjs_dirs | length) > 0 {
                        print $"[OK] Found library directories: ($revealjs_dirs | get name | str join ', ')"
                        $found_lib_files = true

                        # Try to peek into the library structure
                        let first_dir = ($revealjs_dirs | first | get name)
                        try {
