...
The base `isaaclab` output also runs upstream's CPU-only `unit` suite against the installed package (`tests` in `recipe.yaml`). It pulls `omniverseclient` from `pypi.nvidia.com` into the test env (a few `unit` tests import `omni.client`) and excludes the tests that need a GPU or the source checkout, see the [Worth upstreaming](#worth-upstreaming) note. This is a local-first extra: the `pypi.nvidia.com` install keeps it off a clean conda-forge-only run for now.
