...
The base `isaaclab` output also runs upstream's `unit` suite against the installed package (`tests` in `recipe.yaml`). It pulls `omniverseclient` from `pypi.nvidia.com` into the test env (a few `unit` tests import `omni.client`) and excludes only the tests that read the source checkout, see the [Worth upstreaming](#worth-upstreaming) note. The run is device-aware: it detects the GPU with `torch.cuda.is_available()` and runs the `cuda`-parametrized tests (and `deps/test_torch.py`) only on the cuda runners, falling back to the CPU-only subset elsewhere. To install the `manylinux_2_35` `omniverseclient` wheel the test env needs `glibc >= 2.35`, so the output requests `c_stdlib_version 2.39` (AlmaLinux 10, the smallest conda-forge image providing that, via `conda_build_config.yaml` and a `stdlib("c")` build dep) to land on that image; the wheel install stays best-effort, so a missing wheel just drops its two tests. This is a local-first extra.

Before merge: `isaaclab` is pure Python and must go back to `noarch: python`. The `stdlib("c")` build dep, `conda_build_config.yaml` and the per-platform build only exist to force a newer test image for the `omniverseclient` wheel, so they should be removed together (or the unit tests moved to the feedstock) once this lands. All three spots carry a `TODO(before-merge)` marker.
