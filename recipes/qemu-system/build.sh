...
main() {
  ...
  [[ "${target_platform}" != osx-* ]] && QEMU_CONFIGURE_ARGS+=("--enable-strip")
  [[ "${target_platform}" == osx-* ]] && QEMU_CONFIGURE_ARGS+=(
    "--disable-pvg"  # TODO: macos 12 for pvg in feedstock
    "--disable-hvf"  # TODO: revisit -- retained pending a separate HVF/codesign trial once green PR
    "--disable-strip"
  )
...
build_install_qemu() {
  ...
  pushd "${build_dir}" || exit 1
    ${SRC_DIR}/qemu_source/configure \
      ...
      --disable-download \
      "${qemu_args[@]}" \
      > "${SRC_DIR}"/_configure.log 2>&1 || { cat "${SRC_DIR}"/_configure.log; exit 1; }
