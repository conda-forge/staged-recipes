...
For athosdev cluster.

These asrun customizations are called through (in asrun configuration file) :

    schema_calcul : plugins.athosdev.modifier

    schema_execute : plugins.athosdev.change_command_line
"""
...
def getCpuParametersLocal(prof):
    ...
    # Fix the number of physical processors (2) & cores per processor (14).
    cpu_mpi, node_mpi, cpu_openmp, blas_thread = getCpuParameters(2, 14, prof)
...
def change_batch_parameters(serv, prof):
    ...
    batch_custom += (' --cpus-per-task={0} --threads-per-core=1 '
                     ...
                     '--mem-bind=local').format(prof['ncpus'][0])
...
# NOT USED ANYMORE - SET TOO MANY THREADS
