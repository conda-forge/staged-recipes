...
if false; then
# start by recreating a subset of the Makefile.am that will know how to rebuild the tests
# and only the tests.  Did this by manually copy-pasting the Makefile.am
cat >Makefile.am <<-'END_AM'
...
cat >Makefile.am <<-'END_MAKE'
VPATH = $(PREFIX)/share/doc/wolfssl/example/

# everything in the examples directory with a .c is a
# stand-alone program that should be built into an executable
EXAMPLES = $(notdir $(basename $(glob $(VPATH)/*.c)))
