...
. Uses a single nushell script (+build.nu+) for cross-platform build automation
. Configures with CMake (client library only)
. Enables Tracy profiling features
. Installs headers and CMake configuration files
. Automatically detects platform and handles Windows/Unix path differences
. Does NOT include the Tracy profiler GUI (separate package)
