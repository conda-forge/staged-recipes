...
cmake -DUSE_CUDA=ON -DUSE_CONDA_INCLUDES=ON -DUSE_EXTERNAL_THRUST=OFF -DUSE_EXTERNAL_DLPACK=ON -DEXTERNAL_DLPACK_PATH=$PREFIX/include -DUSE_EXTERNAL_DMLC=OFF -DUSE_EXTERNAL_METIS=OFF -DUSE_EXTERNAL_PHMAP=OFF -DUSE_EXTERNAL_XBYAK=OFF -DUSE_EXTERNAL_NANOFLANN=OFF  -DUSE_LIBXSMM=OFF -DUSE_OPENMP=ON -DCUDA_ARCH_NAME=All ${CMAKE_FLAGS} ${CUDA_CMAKE_OPTIONS} ${SRC_DIR}
make -j$CPU_COUNT
cd ../python
