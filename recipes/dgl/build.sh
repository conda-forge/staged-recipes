...
export DLPACK_INCLUDE_DIR=$BUILD_PREFIX/include

cmake -DLPACK_INCLUDE_DIR=$BUILD_PREFIX/include -DUSE_CUDA=ON -DUSE_CONDA_INCLUDES=ON -DUSE_EXTERNAL_THRUST=OFF -DUSE_EXTERNAL_DLPACK=ON  -DUSE_EXTERNAL_DMLC=ON -DUSE_EXTERNAL_METIS=OFF -DUSE_EXTERNAL_PHMAP=ON -DUSE_EXTERNAL_XBYAK=ON -DUSE_EXTERNAL_NANOFLANN=ON  -DUSE_LIBXSMM=OFF -DUSE_OPENMP=ON -DCUDA_ARCH_NAME=All ${CMAKE_FLAGS} ${CUDA_CMAKE_OPTIONS} ${SRC_DIR}
make -j$CPU_COUNT
cd ../python
