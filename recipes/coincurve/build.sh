...
(cd $(tar ztf m4-latest.tar.gz | head -n 1 | sed 's@/.*@@'); ./configure --prefix=./gnu_tools; make, make install)
export PATH=$PATH:./gnu_tools
(cd $(tar ztf autoconf-latest.tar.gz | head -n 1 | sed 's@/.*@@'); ./configure --prefix=./gnu_tools; make, make install)
