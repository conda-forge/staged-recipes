...
:: Using Visual studio
cmake -G "%CMAKE_GENERATOR%" -D CMAKE_BUILD_TYPE=%CONFIGURATION% -D CMAKE_PREFIX_PATH=%LIBRARY_PREFIX% -D CMAKE_INCLUDE_PATH=%LIBRARY_INC% -D CMAKE_LIBRARY_PATH=%LIBRARY_LIB% -D CMAKE_C_FLAGS_RELEASE="/MT" -D CMAKE_CXX_FLAGS_RELEASE="/MT" -D CMAKE_C_FLAGS_DEBUG="/MTd" -D CMAKE_INSTALL_PREFIX=%LIBRARY_PREFIX% ..
if errorlevel 1 exit 1
